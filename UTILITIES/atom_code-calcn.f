C----ATOM_CODE_CALCN.F                                             3/15/06 
C
C----Program to use the molpak.xyz format coordinates and table of atom codes, 
C     volumes and sigmas to calculate a density.  Codes have been extended to
C     include multiple fragment structures, halides anions (no F-), ClO4- 
C     and 1-6 valent C + 3-6 valent B (to handle carboranes).      2/28/05
C
...
C----File # 24 = atom code and volume data
      DIR = '/export/software/predictions-100_14jun05/UTILITIES/atom_cod
     2e_volume.data'
...
745      FORMAT (' Predicted density from atom_code volume ',
     ...
     3           '   |  Prof. Herman Ammon'/
     4           '   |  Dept. of Chemistry & Biochemistry'/
     5           '   |  University of Maryland'/
     6           '   |  College Park, MD 20742 U. S. A.'/
     7           '   |  phone = 301-405-1824'/
     8           '   |  fax = 301-314-9121'/
     9           '   |  e-mail = hlammon@umd.edu'/ 
     A           I5,' atom_codes read and copied to fort.26')
C
C----Read atom id's and xyz's from molpak.xyz file
        open (unit=30,file='molpak.xyz',status='old')
        NATOMS = 0
118     NATOMS = NATOMS + 1
        READ (30,125,END=119) LABEL(NATOMS)(1:2), (XYZ(J,NATOMS),J=1,3)
...
C
            IF (LABEL(I)(1:1) .EQ. 'S' .AND.        ! S-H = 1.5; ! 3/15/06 
     X          LABEL(J)(1:1) .EQ. 'H') DMAX = 1.50 ! 
            IF (LABEL(J)(1:1) .EQ. 'S' .AND.        ! 
     X          LABEL(I)(1:1) .EQ. 'H') DMAX = 1.50 ! 
C
